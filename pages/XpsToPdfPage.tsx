...
export const XpsToPdfPage: React.FC<XpsToPdfPageProps> = ({ onNavigate }) => {
  const [xpsFile, setXpsFile] = useState<File | null>(null);
  ...
  const handleFileChange = async (files: FileList | null) => {
    ...
    const isXps = fileName.endsWith('.xps') || file.type === 'application/vnd.ms-xpsdocument';

    if (!isXps) {
      setError("Invalid file type. Please upload a .xps file.");
      return;
    }
  ...
  const handleConvert = async () => {
    ...
    try {
        ...
        const zip = await JSZip.loadAsync(xpsFile);
        const parser = new DOMParser();

        setProgress('Reading document structure...');
        const fdseqFile = zip.file(/fixeddocumentsequence\.fdseq$/i)[0];
        if (!fdseqFile) throw new Error('Invalid XPS: FixedDocumentSequence.fdseq not found.');
        ...
        if (!fdocPath) throw new Error('Invalid XPS: Document path not found.');

        const fdocFile = zip.file(fdocPath);
        if (!fdocFile) throw new Error(`Invalid XPS: FixedDocument not found at ${fdocPath}`);
        ...
        const pagePaths = Array.from(pageContentTags).map(tag => {
            ...
            // Handle relative paths from the document file
            return source.startsWith('/') ? source.substring(1) : `${fdocDir}/${source}`;
        }).filter((p): p is string => p !== null);

        if (pagePaths.length === 0) throw new Error('No pages found in the XPS document.');
        ...
        for (const [index, pagePath] of pagePaths.entries()) {
            ...
            // Simplified image handling: find the first image and use it as a background
            // A full implementation would need to parse Path geometries and transforms.
        ...
        const originalName = xpsFile.name;
        ...
        console.error(err);
        setError(err instanceof Error ? err.message : "Failed to convert XPS file. The file may be corrupt or use unsupported features.");
    } finally {
  ...
  const renderContent = () => {
        ...
        return (
             <div className="w-full max-w-lg">
                <div className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 md:p-8 text-center">
                    <FileTextIcon className="w-16 h-16 text-purple-400 mx-auto mb-4" />
                    <p className="font-bold text-black dark:text-white truncate mb-1" title={xpsFile.name}>{xpsFile.name}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">{formatBytes(xpsFile.size)}</p>
    ...
    return (
        <FileUpload 
            ...
            title="Drag & Drop Your XPS File Here"
            accept=".xps,application/vnd.ms-xpsdocument"
            description="Supports .xps files."
        />
    )
  }
  ...
  return (
    <div className="w-full max-w-4xl flex flex-col">
        ...
        <div className="w-full flex flex-col items-center justify-center">
            <h1 className="text-3xl md:text-4xl font-bold mb-2 text-black dark:text-white">XPS to PDF Converter</h1>
            <p className="text-md md:text-lg text-gray-500 dark:text-gray-400 mb-8 max-w-2xl text-center">
                Convert your XPS documents into a high-quality PDF document.
            </p>
