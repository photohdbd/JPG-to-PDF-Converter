    ...
    <div className="w-full max-w-2xl bg-gray-800 p-6 rounded-lg border border-gray-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select value={position} onChange={e => setPosition(e.target.value as Position)} className="w-full p-2 bg-gray-700 rounded">
             ...
             <div>
                ...
                <input type="text" value={format} onChange={e => setFormat(e.target.value)} className="w-full p-2 bg-gray-700 rounded"/>
                <p className="text-xs text-gray-500 mt-1">{`{p} = page, {n} = total pages`}</p>
            </div>
            <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Font Size ({fontSize}pt)</label>
                <input type="range" min="8" max="48" value={fontSize} onChange={e => setFontSize(parseInt(e.target.value))} className="w-full accent-brand-secondary"/>
            </div>
             <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Margin ({margin}pt)</label>
                <input type="range" min="18" max="72" value={margin} onChange={e => setMargin(parseInt(e.target.value))} className="w-full accent-brand-secondary"/>
            </div>
        </div>
        <button onClick={handleProcess} className="w-full bg-brand-primary text-white font-bold py-3 mt-8 rounded-lg hover:bg-brand-secondary">
            Add Page Numbers
        </button>
    </div>
  );
  ...
  return (
    <div className="w-full max-w-4xl flex flex-col">
      ...
      <div className="w-full flex flex-col items-center justify-center">
        <h1 className="text-3xl md:text-4xl font-bold mb-2 text-white">Add Page Numbers to PDF</h1>
        <p className="text-md md:text-lg text-gray-400 mb-8 max-w-xl text-center">
            Easily insert page numbers into your PDF documents. Customize position, format, and style for a professional look.
        </p>
        {error && (
        ...
        {resultUrl ? (
            <DownloadScreen pdfUrl={resultUrl} onStartOver={reset} fileName="numbered.pdf" />
        ) : !pdfFile ? (
