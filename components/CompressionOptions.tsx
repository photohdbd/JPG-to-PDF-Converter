...
const OptionCard: React.FC<OptionCardProps> = ({ title, description, level, selectedLevel, onClick }) => {
  ...
  return (
    <div
      ...
      className={`p-6 rounded-xl border-2 cursor-pointer transition-all duration-200 relative ${isSelected ? 'bg-brand-primary/20 border-brand-primary' : 'bg-gray-800/50 border-gray-700 hover:border-brand-secondary'}`}
    >
      {isSelected && (
...
export const CompressionOptions: React.FC<CompressionOptionsProps> = ({
  ...
  return (
    <div className="w-full max-w-4xl bg-gray-900/50 border border-gray-700 rounded-2xl p-6 md:p-8">
      <div className="flex items-center gap-4 mb-6 pb-6 border-b border-gray-700">
        <FileTextIcon className="w-10 h-10 text-red-400 flex-shrink-0" />
        <div>
          <p className="font-bold text-white truncate" title={fileName}>{fileName}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div
          ...
          className={`p-6 rounded-xl border-2 cursor-pointer transition-all duration-200 ${level === 'custom' ? 'bg-brand-primary/20 border-brand-primary' : 'bg-gray-800/50 border-gray-700 hover:border-brand-secondary'}`}
        >
          {level === 'custom' && (
            <div className="absolute top-3 right-3 text-brand-primary">
              <CheckCircleIcon className="w-6 h-6" />
          ...
          <h3 className="text-lg font-bold text-white">Custom Compression</h3>
          <p className="text-sm text-gray-400 mt-1">Manually set image quality.</p>
